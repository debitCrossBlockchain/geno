...
pub fn execute<E:BlockEnv>(
    ...
    let system_contract = match SYSTEM_CONTRACT_FACTORY_INSTANCE.get(){
        Some(s) => s,
        None => return Err(VmError::InternalError { error: format!("can not find system contract {:?}", transaction.to()) }),
    };

    let tx_hash = String::from_utf8(transaction.hash().to_vec()).unwrap();
    ...
    let _ = match system_contract.invoke(        
        ...
        &tx_hash){
            Ok(_) => (),
            Err(e) => return Err(VmError::VMExecuteError { 
                hash: tx_hash, message:e.to_string() }),
        };
