...
pub fn proto_to_json(message: &dyn Message) -> serde_json::Value {
    ...
    for field in message.descriptor().fields() {
        match field_to_json(message, field) {
            Some(x) => map.insert(field.name().to_string(), x),
            None => continue,
        };
    }
...
fn extract_vec_shim<'a, T>(
    ...
    extract_one_fn: &dyn Fn(&'a dyn Message, usize) -> &'a T,
) -> Vec<&'a T> {
...
fn repeated_entry_as_value(pbval: &dyn protobuf::reflect::ProtobufValue) -> serde_json::Value {
    ...
    match pbval.as_ref() {
        ...
        ProtobufValueRef::F32(x) => {
            Value::Number(Number::from_f64(x as f64).expect("repeated f32"))
        }
...
fn singular_field_to_json(
    ...
    match field_descriptor.proto().get_field_type() {
        ...
        FieldDescriptorProto_Type::TYPE_UINT32 | FieldDescriptorProto_Type::TYPE_FIXED32 => {
        ...
        FieldDescriptorProto_Type::TYPE_MESSAGE => {
            let sub_message: &dyn protobuf::Message = field_descriptor.get_message(message);
            proto_to_json(sub_message)
        }
        FieldDescriptorProto_Type::TYPE_ENUM => {
            serde_json::Value::String(field_descriptor.get_enum(message).name().to_string())
        }
