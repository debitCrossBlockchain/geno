...
pub enum Error {
    ...
    InvalidKey,
    /// the iv of sm4 must be 16-byte long
    #[error("the iv of sm4 must be 16-byte long")]
    InvalidIvLength,
}
...
impl Sm4CipherMode {
    ...
    pub fn encrypt(&self, data: &[u8], iv: &[u8]) -> Result<Vec<u8>,Error>  {
        if iv.len() != 16 {
    ...
    pub fn decrypt(&self, data: &[u8], iv: &[u8]) -> Result<Vec<u8>,Error>  {
        if iv.len() != 16 {
            //panic!("the iv of sm4 must be 16-byte long");
            return Err(Error::InvalidIvLength);
        }
    ...
    fn cfb_encrypt(&self, data: &[u8], iv: &[u8]) -> Result<Vec<u8>,Error>  {
    ...
    fn cfb_decrypt(&self, data: &[u8], iv: &[u8]) -> Result<Vec<u8>,Error>  {
        let block_num = data.len() / 16;
        let tail_len = data.len() - block_num * 16;
    ...
    fn ofb_encrypt(&self, data: &[u8], iv: &[u8]) -> Result<Vec<u8>,Error>  {
        let block_num = data.len() / 16;
        let tail_len = data.len() - block_num * 16;
    ...
    fn ctr_encrypt(&self, data: &[u8], iv: &[u8]) -> Result<Vec<u8>,Error> {
        let block_num = data.len() / 16;
        let tail_len = data.len() - block_num * 16;
    ...
    fn cbc_encrypt(&self, data: &[u8], iv: &[u8]) -> Result<Vec<u8>,Error>   {
        let block_num = data.len() / 16;
    ...
    fn 
    cbc_decrypt(&self, data: &[u8], iv: &[u8]) -> Result<Vec<u8>,Error>  {
        ...
        //assert!(last_u8 <= 0x10 && last_u8 != 0); //if the part inside () is not true, the program will panic. 
